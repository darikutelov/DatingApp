...
public class MessageHub : Hub
{
    ...
    public override async Task OnConnectedAsync()
    {
        ...
        var otherUser = httpContext.Request.Query["user"]; // query string
        var groupName = GetGroupName(Context.User.GetUsername(), otherUser);

        // create group with the two users
    ...
    // Makes the usernames alphabetical in a pair
    private string GetGroupName(string caller, string other)
    {
        var stringCompate = string.CompareOrdinal(caller, other) < 0;

        return stringCompate ? $"{caller}-{other}" : $"{other}-{caller}";
    }
